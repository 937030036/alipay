...
def getsn():
    with open('alipayCertPublicKey_RSA2.crt', "r") as f:
        alipay_public_cert_buf = f.read()
        pass
    with open('appCertPublicKey_2021000117696885.crt','r')as f:
        app_public_cert_buf=f.read()
        pass
    with open('alipayRootCert.crt',"r") as f:
        alipay_root_cert_buf=f.read()
        pass
    payobj=AliPayDemo(app_public_cert_buf,alipay_public_cert_buf,alipay_root_cert_buf)
    app_cert_sn=payobj.app_cert_sn
    alipay_root_cert_sn=payobj.alipay_root_cert_sn
    return [app_cert_sn,alipay_root_cert_sn]
